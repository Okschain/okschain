Okschain Core version 0.13.2 is now available from:

  <https://download.okschain.org/okschain-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/okschain-project/okschain/issues>
...
updates will be released anymore. Without security updates, using a okschain
...
ECDSA signatures inside Okschain transactions now use validation using
...
Previous versions of Okschain Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Okschain Core was to raise the default minimum
relay fee.

Okschain Core 0.13.2 will have a strict maximum size on the mempool. The
...
Okschain Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running okschaind without having to do any manual configuration.
...
Okschain Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Okschain Core will relay transactions with
...
entirely in Okschain Core 0.13, and it is at this time undecided whether the
...
Okschain Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Okschain Core automatically creates a hidden service to listen on, without
manual configuration. Okschain Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Okschain Core is listening, and
...
Okschaind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Okschain
Core to use floating fees. By default, Okschain Core will use floating
fees.
...
(default: `0.0002` OKS/kB).

At all times, Okschain Core will cap fees at `-maxtxfee=<x>` (default:
0.10) OKS.
Furthermore, Okschain Core will never create transactions paying less than
...
`okschain.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 28635 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 28635 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /okschainrpc>
        ProxyPass http://127.0.0.1:8635/
        ProxyPassReverse http://127.0.0.1:8635/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass okschaind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< okschainrpc:<password>
    </Location>
...
Restarting okschaind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in okschain.conf
...
okschain-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/okschain-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Okschain Core code base has
...
- `okschain-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `okschain-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Okschain scripting system that in combination with
...
Okschain transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Okschain Core.
...
The two phases are distinct in the Okschain-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Okschain wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Okschain.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Okschain block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of okschains they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of okschains being spent is exactly
...
- **Increased security for multisig:** Okschain addresses (both P2PKH addresses
  ...
  believed to be provided by Okschain's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Okschain's current ledger (called the UTXO set),
  ...
  note that Okschain Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Okschain users to individually opt-in to almost any change in the Okschain
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Okschain Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Okschain
client to Okschain Core 0.13.2 to signal for segwit support. When a
...
Since Okschain Core 0.10.0, nodes have defaulted to only relaying and
...
- `okschain-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `okschain-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
